...
Table~\ref{tab:layerwise} lists the mean cosine between per-domain and
global directions at every layer of Qwen~2.5~3B, for all five conditions
reported in the paper. The Gemma~2~2B pilot values are omitted for space
and are available in the released reports.
...
\begin{table}[t]
  ...
  \begin{tabular}{rccccc}
    \toprule
    & \multicolumn{2}{c}{Refusal (prompt)} & Refusal (resp.) & \multicolumn{2}{c}{Honesty} \\
    \cmidrule(lr){2-3}\cmidrule(lr){4-4}\cmidrule(lr){5-6}
    Layer & Instr & Base & Instr & Instr & Base \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption{Mean cosine between per-domain and global directions by layer
  (Qwen~2.5~3B). ``prompt''/``resp.'' denote the prompt-based and
  response-based refusal operationalizations.}
  \label{tab:layerwise}
\end{table}
