...
steering, also fails, and it fails in the direction favorable to
universality (Table~\ref{tab:steering}). In every domain the global direction shifts
...
to estimation noise, since each per-domain direction is fit on roughly a
quarter of the data; this is itself an argument for pooling. Two caveats:
absolute effects are modest at this coefficient, and the violence domain is
behaviorally immovable (baseline refusal 0.95, unchanged under every
direction), suggesting refusal of violent requests is enforced more
redundantly than a single rank-one intervention can undo.
...
fragmentation may be partly lexical, since arithmetic text simply differs
from trivia text at the surface; this is why we lean on late-layer and
...
out not to generalize. The constructive summary is a scoping rule: a concept direction is universal
