...
behavior at least as well as domain-specific ones; and probes transfer
...
pairs. High off-diagonal accuracy despite low direction cosines would mean
the domains share a decodable concept subspace and the geometric
fragmentation is superficial; off-diagonal accuracy near chance, with high
on-diagonal accuracy, would mean the fragmentation is functional. We report
this transfer matrix for both concepts.
...
counterparts at cosine 0.977 or above, and the dispersion statistic changes
...
others (59 pairs), we repeated the analysis with every domain capped to that
size; the minimum mean cosine barely moves (0.536), so the fragmentation is
not an artifact of unequal sample sizes.
...
redundantly than a single rank-one intervention can undo. The ordering in
Table~\ref{tab:steering} does not depend on the keyword detector: re-scoring
the same generations with an instruction-tuned model as a refusal judge
shifts the absolute rates but preserves the sign of every
own-minus-global comparison, so the global direction remains at least as
effective as each domain's own under both scorers.
