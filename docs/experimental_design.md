...
| # | Claim | Data | Status |
|---|---|---|---|
| **E1 (primary)** | Honesty directions fragment across domains | TruthfulQA (single source, real) | clean ✅ |
| **E2** | Refusal directions fragment | **prompt-based** (harmful vs harmless) | confound-controlled ✅ |
| **E2′ (robustness)** | Same, response-based | `refusal_new/` diversified | secondary |
| **E3** | RLHF amplifies fragmentation | E1/E2 on Base vs Instruct | needs both checkpoints |
| **E4 (behavioral)** | Global steering < per-domain steering | held-out prompts | needs GPU |
